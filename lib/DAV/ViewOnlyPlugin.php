...
namespace OCA\Files_PDFViewer\DAV;
...
class ViewOnlyPlugin extends ServerPlugin {
	...
	private $cacheDownloadPermission = [];
	...
	public function initialize(Server $server): void {
		$this->server = $server;
		//priority 80 to make sure the plugin is called before
		//DAV\ViewOnlyPlugin
		//Sabre\DAV\CorePlugin::httpGet
		$this->server->on('method:GET', [$this, 'checkViewOnly'], 80);
		$this->server->on('method:GET', [$this, 'checkViewOnly'], 95);
	}
	...
	public function checkViewOnly(RequestInterface $request): bool {
		...
		try {
			...
			if (isset($this->cacheDownloadPermission[$path])) {
				$attributes->setAttribute('permissions','download', false);
				return true;
			}
			// Check if read-only and on whether permission can download is both set and disabled.
			// Then force it to enabled for PDF files.
			$canDownload = $attributes->getAttribute('permissions', 'download');
			if ($canDownload !== null && !$canDownload && preg_match('/\.pdf$/', $path)) {
				$attributes->setAttribute('permissions','download', true);
				$this->cacheDownloadPermission[$path] = false;
			}
		} catch (NotFound $e) {
			// File not found
		}
